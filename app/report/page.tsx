...
const MOCK_COMPLAINTS: ComplaintRead[] = [
  ...
  },
  {
    ...
    description: "Children's play area needs cleaning and equipment repair",
    lat: 24.8700,
    lng: 67.0500,
    address: "Seaview Park, Clifton",
    area_code_na: "NA-247",
    area_code_ps: "PS-110",
    status: "resolved",
...
export default function DashboardPage() {
  ...
  // Enhanced fetch with timeout and mock data fallback
  ...
  async function fetchComplaints() {
    ...
    setError(null)
    try {
      const res = await fetchWithTimeout(`${API_BASE}/complaints`, { 
        cache: "no-store",
        headers: {
  ...
  async function fetchImpactStats() {
    try {
      const res = await fetchWithTimeout(`${API_BASE}/impact`)
      ...
      console.warn('Using mock impact stats:', e)
  ...
  async function vote(complaintId: number, value: 1 | -1) {
    ...
    setError(null)
    try {
      const res = await fetchWithTimeout(`${API_BASE}/complaints/${complaintId}/vote`, {
        method: "POST",
  ...
  async function seedExample() {
    ...
    setError(null)
    try {
      const res = await fetchWithTimeout(`${API_BASE}/seed/example`, { 
        method: "POST",
        headers: { "Content-Type": "application/json" }
      })
  ...
  async function createComplaint() {
    if (usingMockData) {
      setError("Complaint creation disabled in demo mode. Start backend server to submit real complaints.")
      return
    }
    ...
    try {
      ...
      if (!payload.title || Number.isNaN(payload.lat) || Number.isNaN(payload.lng)) {
        throw new Error("Please provide title, valid latitude, and longitude")
      }
          ...
          <div className="container mx-auto px-4 py-8">
            <div className="flex items-center justify-between">
              <div>
                ...
                {usingMockData && (
                  <div className="mt-2 p-2 bg-yellow-100 border border-yellow-300 rounded-md max-w-md">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-8">
            <Card className="glass-effect border-2 border-green-200 hover-lift">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-green-700">
                  {impactStats?.issues_resolved?.toLocaleString() ?? "15,247"}
                </div>
            ...
            <Card className="glass-effect border-2 border-green-200 hover-lift">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-green-700">
                  {impactStats?.active_users?.toLocaleString() ?? "51,289"}
                </div>
                ...
                {!usingMockData && (
                  <div className="p-3 bg-green-50 border border-green-200 rounded-md">
                    <p className="text-green-700 text-sm">
                      <strong>Backend Connected!</strong> All features are available.
                    </p>
                  </div>
                )}
              </CardContent>
          ...
          {/* Error Display */}
          {error && (
            <div className="mb-6 p-4 rounded-md border bg-red-50 border-red-300 text-red-700 flex items-start gap-2">
          ...
          <Card className="glass-effect border-2 border-green-200">
            <CardHeader>
              ...
              <CardDescription>
                Latest community reports - vote to prioritize urgent issues
                {usingMockData && " - Voting disabled in demo mode"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                ...
                {usingMockData && (
                  <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded">
                    <p className="text-blue-700 text-xs">
                      <strong>Backend Setup Required:</strong> Run{' '}
                      <code className="bg-blue-100 px-1 py-0.5 rounded text-xs">uvicorn app:app --reload --port 8000</code>{' '}
